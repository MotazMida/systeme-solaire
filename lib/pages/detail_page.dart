...
class _DetailPageState extends State<DetailPage> {
  List<String> categories = ['عرض 360', 'معلومات', 'المعرض'];
                ...
                child: Row(
                  ...
                  children: [
                    Column(
                      children: [
                        Text(
                          'الحجم', // "size" translated to Arabic
                          style:
                              proportional.copyWith(color: blue, fontSize: 18),
                        ),
                        const SizedBox(height: 20),
                        Text(
                          'ق = ${NumberFormat('###,000.#', "ar").format(planet.size).replaceAll('.', ' ')} كم',
                    ...
                    Column(
                      children: [
                        Text(
                          'المسافة عن الشمس', // "distance from Sun" translated to Arabic
                          style:
                              proportional.copyWith(color: blue, fontSize: 18),
                        ),
                        const SizedBox(height: 20),
                        Text(
                          '${NumberFormat('###,000.#', "ar").format(planet.distanceFromSun).replaceAll('.', ' ')} كم',
                          style: montserrat.copyWith(
                              color: white, fontWeight: FontWeight.bold),
                        )
                      ],
